 ...
 */
exports.call = function (websites, header) {
  ...
  websites.forEach(website => {
    ...
    if (website && header) {
      callQueue.add(() => callChain(website, header));
    }
  });
};

function callChain(website, header) {
  return new Promise((resolve) => {
    for(let coref of corefConnection) {
      const callerLog = 'CoRef(' + coref.location + ')';

      console.log('Call ' + callerLog);
      coref.queue.add(() => postRequest(coref.location, website, config.coRefAlgorithms.timeOut))
        .catch((error) => {
    ...
    if (corefConnection.length === 0) {
      // call all given relationship algorithms
      for(let rel of relConnection) {
        callAlgorithm(website, rel, 'Relationships', config.relAlgorithms.timeOut, dbConnection.writeRelationships);
      }
      // call all given date extraction algorithms
      // now check here the header of the content and see whether this call is required or not
      /**
       ...
       * */
        parseHeader(header)
          .then(entity => {
            for (let date of eventConnection) {
              callAlgorithmEvent(website, date, 'DateExtraction', config.eventAlgorithms.timeOut, dbConnection.writeEvents, entity);
            }
          }, error => {
            console.error(error);
          });
...
exports.callSemilar = function (text1, text2) {
  return new Promise((resolve, reject) => {
